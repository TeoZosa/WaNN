...
class TensorFlowDNNRegressor(TensorFlowEstimator, _sklearn.RegressorMixin):
    ...
    def __init__(self, hidden_units, n_classes=0, batch_size=32,
                 ...
                 verbose=1, dropout=None, activation = nn.relu):
    ...
    def _model_fn(self, X, y):
        return get_dnn_model(self.hidden_units,
                                    models.linear_regression,
                                    dropout=self.dropout, activation=self.activation)(X, y)
    @property
    def weights_(self):
...
def get_dnn_model(hidden_units, target_predictor_fn, dropout=None, activation=nn.relu):
    ...
    def dnn_estimator(X, y):
        ...
        layers = dnn(X, hidden_units, activation=activation,dropout=dropout)
