...
def expand_descendants_to_depth_wrt_NN(unexpanded_nodes, without_enumerating, depth, depth_limit, sim_info, lock, policy_net): #nodes are all at the same depth
    ...
    while depth < depth_limit and not over_time and len(sim_info.game_tree)<10000:
        ...
        if len(unexpanded_nodes) > 0 and len(unexpanded_nodes)<2048: #if any nodes to expand;
            ...
            multiprocess = True
            if multiprocess and ((len(unexpanded_nodes) >= 25 and (unexpanded_nodes[0].color == 'White' or not without_enumerating)) or len(unexpanded_nodes) >=100):#lots of children to append
        ...
        if time() - sim_info.start_time < sim_info.time_to_think and len(sim_info.game_tree)<10000:  # stop updating parents as soon as we go over our time to think
            ...
            if not abort:
                ...
                if depth+1 < depth_limit:  # if we are allowed to enter the outermost while loop again
                    best_child = None
                    with lock:
                        if parent.color == 'Black':  # black move
                            best_child = None
...
def update_and_prune(parent, NN_output,sim_info, lock):
    ...
    ranks_to_consider = num_to_check_for_legality + 10 #just in case some top choices are illegal moves or within 10%
        ...
        if height % 2 == 0:  # white moves
            ...
            if height >= 70:
                if parent.reexpanded:
                    ...
                    num_top_to_consider =  2  # else play with child val threshold? WaNN missed an easy win and thought it was doomed at height 53
                # 52-60
                ...
                if parent.reexpanded:
                    ...
                    num_top_to_consider =  3#3  # else play with child val threshold? WaNN missed an easy win and thought it was doomed at height 53
                # 3?   40-51
    ...
    with lock:
        ...
        if len(pruned_children) > 0:
            if parent.children is None:
                ...
                if len (parent.children) >= get_reexpanded_amount(parent): # Virtually reexpanded;  else may try to reexpand and thrash around in tree search
                    reset_reexpansion_flag(parent)
